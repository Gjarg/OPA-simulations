...
def KTA(self, lin):  # , ax = None):
    l = lin*1e6

    if ax == 'nx':

        nx = np.sqrt(2.1495 + 1.0203*l**1.9951/(l**1.9951 -
                                                0.042378) + 0.5531*l**1.9567/(l**1.9567-72.3045))
        return nx

    elif ax == 'ny':

        ny = np.sqrt(2.1308 + 1.0564*l**2.0017/(l**2.0017-0.042523) +
                     0.6927*l**1.7261 / (l**1.7261-54.8505))
        return ny

    elif ax == 'nz':

        nz = np.sqrt(2.1931 + 1.2382*l**1.8920/(l**1.8920 -
                                                0.059171) + 0.5088*l**2/(l**2-53.2898))
        return nz

    else:

        raise Exception(
            "The input is incorrect, ax must be equal to 'nx' or 'ny' or 'nz' ")
